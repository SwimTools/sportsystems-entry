<?php
// Entry Viewer - Withdrawal Version v2.01.2
...
function sport_systems_withdrawel($atts = array(), $content = null, $tag){
...
$withdrawals_form_url = $sport_systems_options['withdrawals_form_url_2']; // Withdrawals form URL
...
$entries_headereventwithdraw = $sport_systems_options['entries_headereventwithdraw']; // Entry Information
$entries_withdrawbutton = $sport_systems_options['entries_withdrawbutton']; // Entry Information
...
.table-group tbody td:nth-child(2),
.table-group tbody td:nth-child(4),
.table-group tbody td:nth-child(6) {
    text-align: center;
    font-size: 70%;
}
...
<hr>
	...
	<form name="form" action="<?php echo esc_attr(plugin_dir_url( 'sportsystems-entry' )) ; ?>sportsystems-entry/processwithdrawal.php" method="post" target="_parent" >

            <div id="table-container" class="table-group">
            </div>
        </div>
	<input type="hidden" name="form_url" value="<?php echo esc_attr($withdrawals_form_url) . ',' . esc_attr($currentmeetname) . ',,,'; ?>">
...
?>
            ...
            .append(
                $('<div>').append(
                    $('<table>').append(
                        $('<thead>').append(
                           `<tr>
                                <td><?php echo esc_attr($entries_headereventwithdraw); ?></td>
        ...
        swimmer.events.forEach(evt => {
            ...
            $(`#t${index} > tbody:last-child`).append(
                `<tr>
                    <td><input type="checkbox" id="horns" name="${evt.number}" value="${swimmer.name},${club.name},${swimmer.birthYear},${evt.comp},${evt.name}"></td>
                    <td>${evt.number}</td>
                    <td>${evt.name}</td>
                    <td>${evt.comp}</td>
                    <td>${evt.time}</td>
...
$(`#t${index} > tbody:last-child`).append(
                `<tr>
                    <td colspan="2"><input type="submit" value="<?php echo esc_attr($entries_withdrawbutton); ?>"></td>
...
add_shortcode('sport-systems-withdrawalsviewer', 'sport_systems_withdrawel');
